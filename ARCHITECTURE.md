...
The `install.sh` script links content from this repository into `~/.claude/` using symlinks (or copies on WSL).

### Presets

| Preset | What Gets Linked |
|--------|-----------------|
| `skills` | Skill packs only (default) |
| `core` | Skills + commands + agents |
| `full` | Core + scripts + hooks + workspaces + templates |

### WSL Detection

On WSL, symlinks across the WSL/Windows boundary can be unreliable. The installer detects WSL via `/proc/version` and uses `cp -rf` instead of `ln -sfn`.

### Manifest

Every install writes a manifest to `~/.claude/.managed/claude-code-wsl-setup.json`. The `--uninstall` flag reads this manifest to cleanly remove only what was installed.

### Opt-in Config

Settings, hooks.json, and CLAUDE.md are never replaced unless explicitly requested:
- `--with-settings` / `--with-permissions` — link settings.json
- `--with-hooks-json` — link hooks.json
- `--with-claude-md` — link CLAUDE.md
...
**settings.json** maps events to scripts:
...
### Notification System (WSL-Specific)

The notification hooks use multiple fallback methods:
...
When running multiple agents, notifications identify which agent needs attention using this priority:
1. `CLAUDE_AGENT_NAME` environment variable
2. Git branch name (for worktree setups)
3. Agent number from directory name
...
The `settings.json` includes a permissions block that pre-approves common safe commands:
...
This reduces permission prompts without using `--dangerously-skip-permissions`.
...
| Script | Purpose |
|--------|---------|
| `create-worktrees.sh` | Create git worktrees for parallel agents (WSL-specific) |
