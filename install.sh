...
# WSL detection: symlinks across WSL/Windows boundary can be unreliable
USE_COPY=0
...
MANIFEST_PATH="$MANAGED_DIR/claude-code-wsl-setup.json"
...
Presets:
  ...
  --preset full     Install core + scripts + hooks scripts + workspaces + templates
...
Opt-in config:
  ...
  --with-permissions         Link settings.json into ~/.claude/settings.json (alias for --with-settings)
...
collect_links_for_preset() {
  ...
  case "$preset" in
    ...
    full)
      ...
      links+=("DIR::$CLAUDE_DIR/templates")
      ;;
    *)
  ...
  if [[ "$preset" == "full" ]]; then
    ...
    for t in "$REPO_DIR/templates"/*; do
      [[ -e "$t" ]] || continue
      links+=("LINK::$t::$CLAUDE_DIR/templates/$(basename "$t")")
    done
  fi

  # Explicit opt-ins
  if [[ "$WITH_SETTINGS" -eq 1 || "$WITH_PERMISSIONS" -eq 1 ]]; then
...
PY
  ...
  local legacy
  for legacy in CLAUDE.md settings.json hooks.json commands skills agents scripts hooks workspaces templates; do
