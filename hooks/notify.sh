...
get_agent_id() {
    ...
    if command -v git &> /dev/null && [ -d "${CLAUDE_PROJECT_DIR}/.git" ] || [ -f "${CLAUDE_PROJECT_DIR}/.git" ]; then
...
if grep -qi microsoft /proc/version 2>/dev/null; then
    ...
    powershell.exe -Command "
        ...
        try {
            ...
            # Method 2: Try balloon notification (works on older Windows)
            try {
                ...
                # Method 3: Sound only (always works)
                [System.Media.SystemSounds]::Exclamation.Play()
